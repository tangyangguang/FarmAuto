...
| FeederSchedule | 多个每日计划 | 512B | 4 | 2048B | 每槽保存计划表快照：最多 6 条计划、跳过今日、执行状态 |
| FeederChannelTarget | 手动投喂默认目标 | 256B | 4 | 1024B | 每路默认 targetMode、targetGramsX100、targetRevolutionsX100、targetPulses |
| FeederCalibration | 三路标定 | 256B | 4 | 1024B | 脉冲/圈数/克数换算 |
| RecordIndex | flash 长期记录索引摘要 | 512B | 4 | 2048B | 不保存完整多年日志 |
| SystemMeta | 存储元信息 | 128B | 4 | 512B | layout version、格式化标记 |

总计约 12800B，AT24C128 剩余约 3584B，可作为扩展余量或增加 `FeederToday` 槽位。
...
- 手动投喂默认目标：保存在 `FeederChannelTarget`，供首页/单路手动投喂使用。
