...
- 长期记录导出、压缩、轮转和容量告警策略。
- 容量满时覆盖最旧记录、停止记录还是只告警。

## 当前需要用户确认

| 编号 | 问题 | 选项 | 推荐值 | 推荐原因 |
| --- | --- | --- | --- | --- |
| D1 | ESP32 flash 文件系统分区容量 | 512KB / 1MB / 2MB / 更大 | 1MB 起步，若 flash 容量允许用 2MB | 多年记录需要空间；1MB 是较稳起点，2MB 更从容 |
| D2 | 长期记录分段方式 | 按天 / 按固定大小 / 按天且超大小再切分 | 按天且超大小再切分 | 远程查询按日期自然，单日异常大量记录也能控制文件大小 |
| D3 | 容量满时策略 | 覆盖最旧 / 停止记录 / 只告警 | 覆盖最旧并告警 | 无人值守场景不应停止记录；覆盖前告警，保留最近多年数据 |
| D4 | 导出格式 | JSON / CSV / 二进制 / 多种 | JSON 分页 + CSV 导出 | JSON 适合 Web/API，CSV 适合人工分析 |
| D5 | 普通记录 flush 策略 | 每条立即 flush / 定时批量 / 条数批量 + 关键事件立即 | 条数或时间批量，关键事件立即 | 平衡掉电丢失风险和 flash 磨损 |
| D6 | AT24C 写入寿命告警阈值 | 50% / 70% / 80% / 90% | 70% warning，90% maintenance | 提前维护，避免等到不可恢复 |
| D7 | Flash 容量告警阈值 | 剩余 20% / 10% / 5% | 20% warning，10% maintenance | 给远程导出和维护留时间 |
| D8 | 长期记录是否压缩 | 不压缩 / 简单二进制编码 / 压缩算法 | 首版不压缩，使用紧凑二进制记录 | 降低实现和恢复复杂度；需要时再加导出侧压缩 |
