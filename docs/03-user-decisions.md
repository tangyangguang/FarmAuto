...
用户已基本接受 C1-C4。涉及 API、字段表和较大数据结构的部分，由 Codex 在源码骨架前继续做技术把关；除非发现会影响业务目标、持久化兼容或远程维护体验的重大变化，否则不要求用户逐字段审查。

| 编号 | 确认事项 | 推荐处理 | 主文档 | 辅助文档 | 需要你回复什么 |
| --- | --- | --- | --- | --- | --- |
| C1 | 三个公共库是否按推荐决策进入接口冻结默认值 | 已基本接受，实测项保留为后续校准 | `docs/libs/25-public-library-freeze-decisions.md` | `docs/libs/21-esp32-encoded-dc-motor.md`、`docs/libs/22-esp32-motor-current-guard.md`、`docs/libs/23-esp32-at24c-record-store.md` | 暂无需回复；源码前由 Codex 再做冻结复核 |
...
| C7 | 自动门和喂食器实际 GPIO、LEDC、ADC、I2C 资源 | 已确认目标按 ESP32-WROOM-32E 系列处理；源码前仍需确认最终 PCB 引脚、ADC 衰减和实测资源 | `docs/15-hardware-resource-budget.md` | 两个应用文档和三个公共库文档 | 暂无需回复；如果实际不是 WROOM-32E 再重排引脚表 |
...
| C15 | 实际 ESP32 模组型号 | 已确认按 ESP32-WROOM-32E 系列处理 | `docs/15-hardware-resource-budget.md` | `docs/apps/10-esp32-farmdoor-rewrite-plan.md`、`docs/apps/11-esp32-farmfeeder-rewrite-plan.md` | 暂无需回复 |
...
| C19 | 自动门 INA240A2 是否首版启用 | 已确认：目标 PCB 已装 INA240A2，接 GPIO33；首版软件支持，但必须可通过编译参数或运行配置关闭，未实测前默认关闭 | `docs/apps/10-esp32-farmdoor-rewrite-plan.md` | `docs/prototypes/web/farmdoor-diagnostics.html`、`docs/apps/24-esp32-farmdoor-web-api.md` | 暂无需回复 |
...
| 原始记录保存 | 尽量完整、尽量长，目标按多年考虑 | 首版优先考虑 ESP32 flash 文件系统，AT24C128 不作为多年原始记录主存储 |
| 维护模式直接设置行程 | 支持 | 可输入圈数或脉冲数，并支持小步微调，便于现有设备远程调整 |
| 运行中断电恢复 | 尽量自动恢复 | 使用 motion journal 和低频位置检查点；可信恢复不需要人工或 Web 确认，优先恢复到可正常继续控制状态 |
