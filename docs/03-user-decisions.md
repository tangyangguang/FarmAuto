...
| 原始记录保存 | 尽量完整、尽量长，目标按多年考虑 | 不应受旧方案限制；AT24C128 不适合作为多年原始日志主存储 |
...
| 历史记录保存粒度 | 原始记录尽量完整，时间尽量长，目标按多年考虑 | 不能按旧方案只保留少量历史；需要单独长期记录存储方案 |

## 尚需继续确认

- 自动门故障 `Coast` 的实机安全性。
- 自动门保存位置可信的判定条件和限位交叉校验规则。
- 长期原始记录使用的存储介质：ESP32 flash 文件系统、外部 FRAM、SD 卡或其他方案。
- 长期记录导出、压缩、轮转和容量告警策略。
