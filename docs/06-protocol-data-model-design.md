...
- RS485：默认 `115200 8N1`。
- 波特率作为安装维护参数处理，建议支持 `9600`、`19200`、`38400`、`57600`、`115200` 这些固定档位。
- 主控和所有分站必须使用同一波特率；在线切换波特率需要专门维护流程和回退策略，当前第一阶段不做自动多波特扫描或运行中批量改速。
...
协议目标是高效、健壮、清晰，并保留必要扩展点；不做复杂工业总线框架。

- 当前主业务是单电机执行节点，未来可能增加简单传感器节点或简单执行节点。
- 所有节点复用同一帧格式、地址规则、`PING` 基础识别和 CRC 校验。
- 不同节点用 `deviceClass` 和 `capabilityFlags` 表达能力；主控只绑定和执行已支持的节点类型。
- 核心命令使用固定 payload，便于 STC8H 端低成本解析；当前不引入通用 TLV 业务框架。
- 诊断类或未来低频扩展命令可以在新命令码或新协议版本中使用 TLV，但不影响现有核心动作命令。
- 兼容性依赖 `protocolVersion`、`deviceClass`、`capabilityFlags`、`maxPayloadLen`、命令码分段和保留字段，而不是把当前实现做成大而全。
...
- 地址 `0` 表示主控/保留地址，不绑定普通设备。
- 地址 `1..127` 是普通分站地址，主控默认扫描该范围。
- 地址 `128..254` 保留未来用途。
- 地址 `255` 保留为广播地址；当前不实现广播动作命令，未来即使使用广播也只能用于低风险维护类命令。
- `len` 字段为 1 字节，帧格式理论可表达 `0..255` 字节 payload；当前实现限制 `FA_MAX_PAYLOAD_LEN = 64`，超过即拒绝或丢弃。
- `64` 字节 payload 足够当前电机配置、动作和状态响应，同时避免 STC8H RAM 占用和低波特率场景下总线占用过高。未来如需更长 payload，应通过新协议版本提高实现上限，而不是改变当前帧格式。
...
| `dst` | 1 | 目标地址；主控为 `0`，普通分站为 `1..127` |
| `src` | 1 | 源地址；主控为 `0`，普通分站为 `1..127` |
| `seq` | 1 | 主控递增序号，响应原样返回 |
| `cmd` | 1 | 命令码 |
| `len` | 1 | payload 长度；当前实现最大 `64` |
| `payload` | `len` | 命令数据 |
| `crc16` | 2 | CRC16，小端 |
...
| `0x30..0x3F` | 维护命令 |
| `0x40..0x4F` | 诊断命令 |
| `0x50..0x6F` | 未来简单传感器或简单执行节点预留 |
| `0x70..0x7F` | 生产测试和厂内维护预留 |
| `0x80..0xEF` | 保留 |
| `0xF0..0xFF` | 协议级扩展或特殊保留 |
...
| `deviceClass` | `uint8` | 节点类型 |
| `capabilityFlags` | `uint32` | 能力位 |
| `maxPayloadLen` | `uint8` | 分站当前支持的最大 payload 长度 |

`deviceClass` 当前定义：

| 编码 | 名称 | 说明 |
| ---: | --- | --- |
| `0x00` | `DEVICE_CLASS_UNKNOWN` | 未知或未声明 |
| `0x01` | `DEVICE_CLASS_MOTOR_ACTUATOR` | 单电机执行节点，当前成品支持 |
| `0x02` | `DEVICE_CLASS_SENSOR_NODE` | 未来简单传感器节点预留 |
| `0x03` | `DEVICE_CLASS_SIMPLE_ACTUATOR` | 未来简单执行节点预留 |
| `0x04..0x7F` | 保留 | 后续正式节点类型 |
| `0x80..0xFF` | 厂商/实验保留 | 不作为成品绑定依据 |

`capabilityFlags` 当前定义：

| bit | 名称 | 说明 |
| ---: | --- | --- |
| 0 | `CAP_MOTOR_BIDIRECTIONAL` | 支持双向电机动作 |
| 1 | `CAP_HALL_AB_ENCODER` | 支持霍尔 AB 相编码器 |
| 2 | `CAP_CURRENT_SENSE` | 支持电流检测 |
| 3 | `CAP_BRAKE_SUPPORTED` | 支持刹车停止 |
| 4 | `CAP_IDENTIFY_LED` | 支持识别闪灯 |
| 5 | `CAP_CONFIG_REQUIRED_AFTER_BOOT` | 上电后必须由主控下发配置才允许运行动作 |
| 6 | `CAP_DIAG_SUPPORTED` | 支持 `READ_DIAG` |
| 7 | `CAP_CLEAR_FAULT_SUPPORTED` | 支持 `CLEAR_FAULT` |
| 8..31 | 保留 | 必须上报为 0，主控忽略未知位 |
...
- RS485 默认 `115200 8N1`、`80 ms` 超时、`5` 次失败离线作为联调起点；波特率保留安装维护配置入口，最终按现场总线长度和干扰实测校正。
