...
- 一块分站板只控制一个双向直流减速电机。
- 门控和下料使用同一种分站硬件能力，都是 AT8236 + 编码器 + INA240A2 + RS485 + RUN/ERR LED。
...
- 新从站 PCB 的 AT8236、编码器、INA240A2、地址输入、LED 和 RS485 DE/RE 实际 IO。
...
- 下料：每圈脉冲、每圈克数、单次下料量、手动下料、自动下料计划和错峰启动。
