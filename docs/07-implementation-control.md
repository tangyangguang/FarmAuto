...
- 主控提供 `/api/status/summary` 只读状态摘要，用于验收当前 RS485 模式、动作跟踪、记录计数、设备状态、SHT30 状态和自动计划状态。
- 主控提供 `/api/records/recent?limit=N` 只读动作记录接口，默认返回最近记录，最大 `limit=32`。
- 下料和门控动作 ID 使用 `Esp32BaseConfig` 保存 next id；生成真实动作后立即递增持久化，重启后不复用旧动作 ID。
- `/bus` 支持扫描 `1..127` 地址并把应答分站写入设备注册表。
- 主控后台在无动作运行时低频轮询启用分站，更新在线/离线/错误状态。
- `/auto` 显示每天固定自动计划、时间同步状态、暂停状态和最近动作记录；自动下料、自动开门、自动关门由后台调度器在真实时间同步后按本地日分钟触发。
- 自动计划参数走 `Esp32BaseAppConfig`，暂停/恢复通过 Web API 写入基础库配置；暂停只影响自动执行，手动下料和手动门控不受影响。
- 自动计划不做补跑队列：时间不同步、设备禁用、分站禁用、分站故障、动作忙或 RS485 未配置时跳过本次计划并写日志。
- `/notify` 只保存通知规则并展示通道未接入状态；当前不实现巴法云/微信真实发送、发送队列或外网重试。
- `/env` 使用 ESP32 真实 I2C 总线读取 SHT30 温湿度传感器，默认 SDA=`21`、SCL=`22`、地址 `0x44`，采样间隔和记录间隔走 `Esp32BaseAppConfig`。
- SHT30 采样成功后写入 Esp32Base App Events，事件 `source=farm`、`type=env_sample`；未接传感器时显示真实 I2C 失败状态和失败计数，不使用模拟温湿度。
- `/board` 使用 ESP32 真实 GPIO 驱动主控 RUN/ERR LED，并读取 BOOT 和预留外接按钮输入；本阶段只记录和展示按键事件，未把按钮绑定到门控动作。
- 主控日志覆盖手动动作预览、真实发送、动作跟踪、轮询失败、动作终态、分站状态变化和总线扫描摘要，便于无板和上板阶段对照。

下一步优先继续补齐“可见、可验证”的主控功能闭环；FRAM pending-action journal、真实巴法云/微信发送、分站 OTA、缺料检测和真实板级 IO 联调仍然后置。
