...
为了以后迁移到 FRAM 不返工，记录层应尽量保持设备无关：

```text
RecordStore
  RandomAccessNvMemory
    At24cDevice
    future I2cFramDevice
```

如果未来真实使用 FRAM，推荐新增独立后端或独立库，例如 `Esp32I2cFramRecordStore`，或者再抽取通用 `Esp32RecordStoreCore`。当前阶段不提前改名，也不承诺 FRAM 行为。
