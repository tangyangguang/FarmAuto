...
本文记录三个公共库进入源码骨架后需要满足的测试维度。当前已进入公共库源码阶段，测试按 host 单元测试、PlatformIO manifest 检查和后续硬件在环三层推进。

当前已创建第一批 host 测试，统一入口：

```bash
tools/run_host_tests.sh
```

本文后续继续作为单元测试、硬件在环测试和发布前检查清单使用。
...
- INA240A2 零点校准。
- ESP32 ADC eFuse/校准曲线启用后，空载、正常负载和过载读数误差在可接受范围内。
...
- AT24C32/64/128/256/512 这类 2 字节地址 preset 参数正确。
- 跨页读取和写入正确拆分。
- header 不跨页约束可被校验。
- `Writing` 记录断电后被忽略。
...
- 双阶段提交的写放大有估算记录，确认高频 recordType 的寿命预算仍可接受。
