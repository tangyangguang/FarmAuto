...
- `/calibration`：行程校准，含标定关门基准、用当前位置更新开门目标、直接设置行程和微调行程。开门/关门命令直接驱动 AT8236/PCNT，驱动或编码器未就绪时返回业务错误。
...
- 电机：返回 `motorRuntime.ready`、motor snapshot、encoder、driver output 和 fault；不再提供 `door/motorOutput` 测试开关。
...
- 当前版本支持设置关闭点、保存开门目标、直接设置行程圈数或脉冲数、微调行程；普通开门/关门命令直接执行 AT8236/PCNT 闭环动作。
