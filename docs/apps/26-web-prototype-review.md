...
| `/channels` | 不使用 | 基础信息 | 喂食器通道基础信息、料桶容量和低频业务基础设置 |
...
基础信息页不做实际运转测试。页面一次只编辑一个通道，避免三路参数混在一个大表单中误保存。当前估算余量属于业务运行状态，不放入基础信息编辑表单；余量只能通过补料、标记满桶或修正余量入口更新并记录业务事件。手动下料放在首页，计划自动下料放在计划页。
