...
| 运行状态 | 应用状态机 + AT24C128/flash | 当前位置、计划执行实例跳过状态、今日计数、当前估算余量、最近故障、长期原始记录 |
...
当前代码阶段尚未输出 PWM，也未接入编码器，因此喂食器首版暂不注册这些字段到 Esp32Base App Config。通道启用、每圈信号数、每圈克数和料桶容量已经属于业务“基础信息”页面/API；只有真实电机输出接入后，速度、软启动、软停止、最大运行时间等低频硬件参数才进入 App Config。

这些参数作为后续硬件适配阶段候选。三路喂食器电机虽然同系列，但转速比可能不同，因此速度、软启动、软停止、减速比、输出轴每圈脉冲等都按每路独立配置。
...
| 每路当前估算余量 | 首页料桶余量区 | AT24C128 关键状态 + 长期记录 | 补料和投喂扣减更新，不能放基础信息编辑表单或 App Config |
| 每日计划列表 | 计划页 | 应用持久化服务 | 最多 6 条；每个计划独立启用、时间、参与通道和每路目标；未配置时间时该计划不自动投喂 |
| 跳过本次计划执行 | 计划页 | AT24C128 今日/明日状态 | 只影响具体执行实例，不删除长期计划，日期切换后自动清除过期状态 |
...
- Esp32FarmFeeder 当前暂不注册已实现业务字段到 App Config；构建配置仍保留 `ESP32BASE_APP_CONFIG_MAX_FIELDS >= 96` 余量，供后续真实电机输出、电流检测和少量系统硬件参数使用。
...
- 饲料桶当前估算余量不放 Esp32Base App Config，也不放基础信息编辑表单；通过首页补料、标记满桶或修正余量入口维护。
