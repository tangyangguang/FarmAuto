...
Esp32Base Runtime FileLog 默认写入 `/logs/eb_app.log`，它是系统级运行诊断日志，不是 FarmAuto 的长期业务记录，也不是 FarmAuto 业务事件列表。
...
| 业务事件 | FarmAuto 封装 + Esp32Base 存储 | App Events / 业务事件 | 清故障、清今日、跳过计划、补料、设置余量、基础信息修改、业务告警和业务异常 | `FarmAutoEventLog` 写入 Esp32Base App Events，容量使用默认 1024 |
| 长期业务记录 | 应用层 | Records / 业务记录 | 自动门开/关/停执行历史，喂食器手动/计划投喂和单路完成记录，统计/报表所需数据 | 应用记录服务，ESP32 flash segment + 必要索引 |
...
- 应用代码不得直接散落调用 `Esp32BaseAppEventLog::append()`；所有事件写入和业务化读取都通过 `FarmAutoEventLog`。
- App Events `source` 禁止使用 `wifi`、`ota`、`ntp`、`fs`、`boot`、`esp32base`、`system` 等基础库或系统命名。
...
| `GET /api/app/events/recent` | 从 App Events 读取并映射后的业务事件 |
...
FarmAuto 业务原型只展示应用导航：

- Esp32FarmDoor：首页、记录、校准、诊断。
- Esp32FarmFeeder：首页、计划、记录、基础信息、诊断。

系统日志、OTA、WiFi、系统工具、文件系统格式化等基础库能力由 Esp32Base 自己提供，FarmAuto 业务页不重复做原型、不复制系统日志入口，也不能把业务记录写入 `/logs/eb_app.log`。

## 源码前检查

进入应用源码前必须用 Esp32Base 当前 head 做一次能力验证，不按“未来会支持”推进 FarmAuto 源码。
...
截至 2026-05-16 本轮只读检查，Esp32Base 当前目录已有以下能力证据：
...
- `Esp32BaseWeb` 默认 route 容量为 ESP32 16 个、ESP32-C3 12 个；FarmAuto 应用源码前必须按页面和 API 实际数量核算，必要时在应用构建宏中调整。
