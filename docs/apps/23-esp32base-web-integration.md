...
| `/maintenance` | 维护/校准页 / Maintenance | 应用 |
...
- 长期记录容量告警阈值、是否启用相关功能。
- 喂食器启动全部间隔、保护阈值和硬件启用状态。

不能放入 Esp32Base App Config：

- 当前门位置、行程校准中的临时状态。
- 每日计划时间、每路投喂目标、饲料桶容量、低余量阈值等需要专门交互的业务配置。
- 当前估算余量、补料动作、跳过今日运行态。
- 今日计数、最近故障、最近事件。
- 长期业务记录正文。
- 任何需要高频写入或随业务动作变化的数据。
...
- OTA 是否已有可用的失败恢复或回滚策略；若没有，FarmAuto 文档必须把 OTA 回滚列为发布前风险。
...
- OTA 文档和代码中已有 rollback / mark-valid timeout 能力，但 `ESP32BASE_OTA_REQUIRE_MARK_VALID` 默认值需要在 FarmAuto 构建配置里明确。

FarmAuto 两个应用推荐构建配置：

- `ESP32BASE_OTA_REQUIRE_MARK_VALID=1`。
- `ESP32BASE_OTA_MARK_VALID_TIMEOUT_MS=300000`，即 5 分钟。
- OTA 后首次启动必须完成基本健康检查和业务状态加载，再标记固件有效。
- 如果目标分区表或目标板无法通过 rollback 实测，发布前必须把 OTA 回滚列为阻塞风险，不允许无人值守远程升级。
...
- OTA rollback 是否在目标分区表和目标板上完成实机验证。
