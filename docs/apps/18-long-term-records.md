...
| 当前位置/今日计数/计划执行跳过状态 | AT24C128 | 小型状态，重启后必须可信 |
...
- 使用二进制 segment + 变长事件记录，不使用自由格式文本日志作为主记录。
...
- 每条记录有固定头和变长 payload；payload 按事件类型编码。
- 读取分页时按 `payloadLength` 顺序跳到下一条记录。
- `headerCrc` 只覆盖固定头中除自身外的字段；`payloadCrc` 覆盖 payload。
- 单条记录损坏时丢弃该记录，并尝试从下一个 magic 继续扫描；不要让一个损坏事件导致整个文件不可读。
- 禁止直接把 C++ 结构体内存写入 flash；必须字段级显式序列化，固定 little-endian。

旧的二槽 `value1/value2` 记录模型不再作为首版格式。它会限制 `FeederPowerLossAborted`、`DoorMotionStopped` 等高价值事件的 payload 表达，容易在实现阶段再扩展成私有格式。

通用头语义与事件字段见 `docs/apps/22-record-event-schema.md`，两份文档必须保持一致。
...
如后续确认 Esp32Base 能力不足，应到 Esp32Base 项目处理，不在 FarmAuto 内打补丁。需求草案如下：
