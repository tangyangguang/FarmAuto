...
- 关键事件才立即 flush。
- 按天或固定大小轮转。
- 达到容量上限后优先覆盖最旧 segment 或提示维护，策略需确认。
...
系统应尽早发现存储异常，并通过 Web 状态、日志和维护页面给出告警。
...
- `StorageFault`：关键配置/状态无法读取、AT24C 离线、flash 挂载失败、无法写入新关键状态。
