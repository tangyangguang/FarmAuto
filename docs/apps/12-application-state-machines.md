...
- `Maintenance`：维护模式，允许端点校准等危险操作。端点示教、端点验证、下一阶段上限位寻零都作为 `activeFlow` 表达，不再提升为长期主状态。
...
- 手动启动多个通道或多通道启动时，已忙通道跳过，空闲可用通道可以继续启动；响应中必须返回 successMask、busyMask、faultMask 和 skippedMask。
...
- `Maintenance`：维护模式，允许清空计数、校准等危险操作。
...
- 跳过某个计划执行实例只影响该次自动定时投喂，不影响手动下料。
- 时间无效时暂停自动定时投喂，但允许手动下料；手动下料受通道级 Busy、故障和维护状态限制。
