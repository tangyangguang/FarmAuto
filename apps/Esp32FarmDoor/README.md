...
- 已接入 AT8236 LEDC 双 PWM 输出和 PCNT 编码器适配；真实硬件输出由 Esp32Base App Config 的 `door/motorOutput` 控制，默认关闭。
...
- 返回 `motorOutput` 真实状态、编码器快照、驱动输出和电机故障摘要；默认 `enabled=false`，启用后才会输出 PWM。
...
- `door/motorOutput=false` 时返回业务错误，不输出 PWM；启用后驱动 AT8236 并用 PCNT 闭环到开门目标。

`/api/app/door/close`

- 请求业务状态机进入关门状态。
- 如果位置未可信标定，返回 `PositionUntrusted`。
- `door/motorOutput=false` 时返回业务错误，不输出 PWM；启用后驱动 AT8236 并用 PCNT 闭环到关门目标。
